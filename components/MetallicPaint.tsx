...
float mG(float hi,float lo,float t,float sh,float cv){
  ...
  r=mix(r,hi,smoothstep(e3-sh,e3+sh,t));
...
export default function MetallicPaint({
  ...
  brightness = 2, contrast = 0.5, angle = 0, fresnel = 1, lightColor = '#ffffff',
  darkColor = '#000000', patternSharpness = 1, waveAmplitude = 1, noiseScale = 0.5,
  chromaticSpread = 2, mouseAnimation = false, distortion = 1, contour = 0.2, tintColor = '#feb3ff'
}: MetallicPaintProps) {
  ...
  const uploadTexture = useCallback((imgData: ImageData) => {
    ...
    gl.uniform1f(uniformsRef.current.u_imgRatio, imgData.width / imgData.height);
    gl.uniform1f(uniformsRef.current.u_ratio, 1);
